"""Deterministic test doubles for the backend seam (M1-T5).

``FakeBackend`` derives a stable answer from any question so the whole wire cycle is
testable without a model. ``ScriptedBackend`` replays a fixed set of answers, which is how
the golden fixtures pin the documented response shape.
"""
...
def estimate_usage(state: State, questions: dict[str, Question]) -> Usage:
    """A deterministic, model-free token estimate for tests."""
...
class FakeBackend:
    """Answers every question deterministically; useful for offline contract tests."""
